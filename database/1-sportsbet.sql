...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  `user_lang` varchar(5) NOT NULL DEFAULT 'en',
  ...
  `user_notify` enum('','all','won','cancelled') NOT NULL DEFAULT 'all',
  `user_sendmail` varchar(100) NOT NULL DEFAULT '0' COMMENT '0 = disabled, >0 is category id',
  `user_remind` int(1) NOT NULL DEFAULT '1' COMMENT 'remind user 30 mins b4 game ends',
  `user_gamedigest` int(3) NOT NULL DEFAULT '2' COMMENT '0=disabled, 1=periodically,2=daily',
  `user_sitenews` int(1) NOT NULL DEFAULT '1' COMMENT 'receive news and updates',
  `user_privacy_page` enum('all','following','none') NOT NULL DEFAULT 'all',
  `user_privacy_result` enum('all','following','none') NOT NULL DEFAULT 'all',
  PRIMARY KEY (`user_id`),
